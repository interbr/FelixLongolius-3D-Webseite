...
var con = mysql.createConnection({
  ...
  else if (sen.doing == 9) { con.query("UPDATE `present` SET `online`=1, `uW`='" + sen.hiCo + "', `uH`='" + sen.level + "', `uD`='" + sen.sMMs + "', `turn`='" + sen.turn + "', `turnX`='" + sen.turnX + "', `turnZ`='" + sen.turnZ + "', `duration`='" + sen.duration + "' WHERE `number`='" + sen.whoC + "'"); }
  ...
  function startInterval(){
    ...
    setPerson = "SELECT `number`, `name`, `color`, `room`, `uW`, `uH`, `uD`, `turn`, `turnX`, `turnZ`, `duration`, `conn`, `extra`, `extraContent`, `videoSize`, `audioSes`, `timestamp`, `TA`, `online`, `isAdmin`, `kindOfGuy` FROM `present` WHERE `active` = '1' AND (`online`=1 OR `online`=2) ORDER BY id ASC;";
...
client.send(JSON.stringify({type:"person_moves", data: results[i]}));
  ...
  /* con.query(setPersonInfo, function(err, results){
    if (err){ 
      throw err;
    }
    ws.send(JSON.stringify({type:"person_moves", data: results}));
  }); */
