...
var con = mysql.createConnection({
  ...
  var setActiveFleo;
  const setPerson = "SELECT `number`, `name`, `color`, `room`, `uW`, `uH`, `uD`, `turn`, `duration`, `conn`, `extra`, `extraContent`, `videoSize`, `audioSes`, `timestamp`, `TA`, `online`, `isAdmin`, `kindOfGuy` FROM `present` WHERE `active` = '1' AND (`online`=1 OR `online`=2) ORDER BY id ASC;";
  const getVideo = "SELECT `fleoNum`, `timestampFleo` FROM `thefleos` WHERE `active` = 1;";
  personDataTimestamp = [];
  ...
  if (sen.type !== "ping") {
    ...
    if (sen.type == "video") {
      ...
      clientID.set(ws, sen.fleoNum);
      if (thingsGoing == 0) {
      
      con.query("INSERT INTO `thefleos` (`active`, `online`, `fleoNum`, `lettersOrVideo`, `fleoip`) VALUES (1, 0, '" + sen.fleoNum + "', 0, '" + req.socket.remoteAddress + "') ON DUPLICATE KEY UPDATE `active`=1, `online`=0,`lettersOrVideo`=0, `fleoip`='" + req.socket.remoteAddress + "'");
      con.query("UPDATE `present` SET `online`=1 WHERE `number`='" + sen.fleoNum + "';");
...
client.send(JSON.stringify({type:"person_moves", data: results[i]}));
  ...
  con.query(getVideo, function(err, results){
    if (err) throw err;
    for (i = 0; i < results.length; i++) {
...
let defaultProfile = "";
  ...
  const fleoNum = clientID.get(ws);
  con.query("UPDATE `present` SET `online`=2, `conn`=0 WHERE `number`='" + fleoNum + "';");
  setTimeout(function(){ 
    const fleoNum = clientID.get(ws);   
    con.query("UPDATE `present` SET `online`=3 WHERE `number`='" + fleoNum + "';");
    con.query("UPDATE `thefleos` SET `online`=0,`defleos`='" + defaultProfile + "' WHERE `fleoNum`='" + fleoNum + "' AND `online`=1;");
  }, 1500)
  };
