                                                ...
                                                w = $(window).width(), h = $(window).height(), thisAllScale = w / 1800, $("#wrapperContainer").transition({
                                                    ...
                                                    pointZero = $(window).scrollTop() - parseInt($("body").css("height")), relationToBackground = $(window).scrollTop() - parseInt($("body").css("height")) - pointZero + 35, $("html").animate({
                                                        ...
                                                        $("#wrapper").animate({
                                                            ...
                                                            $("#bgbgsterne").animate({
                                                                left: "0px" // left: ($(window).width() - $("#bgbgsterne").width()) / 2 + "px"
                                                            }, 0, (function () {
                                                                $("html").animate({
                                                                    scrollTop: $("body").css("height")
                                                                }, 0, (function () {
...
function moveWithSocket(coordsTr, doordsTr, duration, height, turn) {
    ...
    if (hp % 1 == 0) {
        window.history.replaceState({
            page: mainDomain
        }, mainDomain, "/r/" + myRoom + "?coords=" + (parseInt(historyCoords)) + "&doords=" + (parseInt(sMMssM / 3)));
    };
    hp++;
...
function connectWorld() {
    ...
    worldSrc.addEventListener('genesis-complete', function (e) {
        if (genesisDone == 0 && firstRoomLoaded == 0) {
            ...
            $("#flash").addClass("fff");
            if (!$("#genesisOverlay").length) {
                $("body").append('<div id="genesisOverlay" style="position:fixed;z-index:99999999;top:0;left:0;width:100%;height:100%;background:#ffffff1f;color:white;text-shadow: -1px -1px 0px #000, 1px -1px 0px #000, -1px 1px 0px #000, 1px 1px 0px #000;font-size:120px;text-align:center;">Loading ...</div>');
            }
            $(".thing").remove();
            $(".floor").remove();
            for (let i = 0; i <= world2GeneratePortionCount; i++) {
...
if (localStorage.getItem('spacenumber') || urlForCoordsGet.has('heronumber')) {
    ...
    myRoom = startRoom;
    $("#h1title").html("Drop holiday photos, say hey: " + mainDomain + ", room: " + myRoom);
    $("title").html("Drop holiday photos, say hey: " + mainDomain + ", room: " + myRoom + " // " + mainDomain + " open-world hubs with content-trees");
