var buildPhpOpen = 0;
var buildBuilderOpen = 0;

if ($("#buildhtml").length) {
window.myCodeMirror1 = CodeMirror.fromTextArea(document.getElementById("buildhtml"), {
    mode: "htmlmixed",
    lineNumbers: true,
    lineWrapping: true,
    matchBrackets: true,
  ...
  window.myCodeMirror1.getWrapperElement().id = "buildhtmlCM";
}
if ($("#buildjavascript").length) {
window.myCodeMirror2 = CodeMirror.fromTextArea(document.getElementById("buildjavascript"), {
    mode: "javascript",
    lineNumbers: true,
    lineWrapping: true,
    matchBrackets: true,
  ...
  window.myCodeMirror2.getWrapperElement().id = "buildjavascriptCM";
}
...
$("#getPhpBuilder").click(function(){
  if (buildBuilderOpen == 0) { 
    buildBuilderOpen = 1; 
  $("#getPhpBuilder").toggleClass("builderClosed builderOpen");
  $("#phpBuilder").show("drop", { direction: "up", easing: "linear", duration: 1000 });
  } else {
    buildBuilderOpen = 0;
    $("#getPhpBuilder").toggleClass("builderClosed builderOpen");
    $("#phpBuilder").hide("fold", { direction: "up", easing: "linear", duration: 1000 });
  }
});
$("#getphp2codeEditor").click();