...
if ($row["coordsW"] > 8000) { $mpMoveW = 0; $moveFragileW = -200; $mpChange = 1; } else if ($row["coordsW"] < 0) { $mpMoveW = 1; $moveFragileW = 200; $mpChange = 1; } else { $mpChange = 0; }
...
if ($row["program"] == $meIam) {
    ...
    */
    ...
    if ($i > 9) { 
        exec('php ' . $fleoPathAbs . '/fleo.at_1.0.0/fleo.at-php/fragiles/streetMen2.php 1 '.$id.' '.$room.' > /dev/null &');
        $goBaby = 0;
    }
    if ($row["program"] !== $meIam) {
        $goBaby = 0;
