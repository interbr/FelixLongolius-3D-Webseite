...
if ($moveFragileD < -400) { $moveFragileD += 200; }
if ($row["coordsD"] < 0) { $moveFragileD = 50; }
...
if ($row["program"] == $meIam) {
    ...
    $set_fragile_while = "UPDATE `$room` SET `coordsW` = `coordsW` + '$moveFragileW', `coordsD` = `coordsD` + '$moveFragileD', `coordsH` = `coordsH` + '$moveFragileH', `tick` = `tick` + 1, `play` = 1, `seek` = '0.01', `minusPlusW` = '$mpMoveW', `mpChange` = '$mpChange' WHERE `whatIsThis` = '$id';"; 
        ...
        $set_fragile_while = "UPDATE `$room` SET `coordsW` = `coordsW` + '$moveFragileW', `coordsD` = `coordsD` + '$moveFragileD', `coordsH` = `coordsH` + '$moveFragileH', `tick` = `tick` + 1, `minusPlusW` = '$mpMoveW', `mpChange` = '$mpChange', `play` = 3 WHERE `whatIsThis` = '$id';"; 

        $set_fragile_laugh = "UPDATE `$room` SET `coordsW` = `coordsW` + '$moveFragileW', `coordsD` = `coordsD` + '$moveFragileD', `coordsH` = `coordsH` + '$moveFragileH', `tick` = `tick` + 1, `minusPlusW` = '$mpMoveW', `mpChange` = '$mpChange', `play` = 1 WHERE `whatIsThis` = '$id';"; 
    // }
    }
    ...
    */
    ...
    if ($i > 79) {
        ...
        exec('php ' . $fleoPathAbs . '/fleo.at_1.0.0/fleo.at-php/fragiles/Birds.php 1 '.$id.' '.$room.' > /dev/null &');
        $goBaby = 0;
    }
    if ($row["program"] !== $meIam) {
        $goBaby = 0;
