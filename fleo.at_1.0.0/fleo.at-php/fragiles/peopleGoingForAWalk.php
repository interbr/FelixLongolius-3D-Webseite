...
if ($moveFragileD < -400) { $moveFragileD += 200; }
if ($row["coordsD"] < 4000) { $moveFragileD = 50; }
if ($row["coordsD"] > 6000) { $moveFragileD = -50; }
...
if ($row["coordsW"] > 2500) { $mpMoveW = 0; $moveFragileW = -1000; $mpChange = 1; } else if ($row["coordsW"] < -2500) { $mpMoveW = 1; $moveFragileW = 1000; $mpChange = 1; } else { $mpChange = 0; }
...
if ($row["program"] == $meIam) {
    ...
    if ($i == 1) {
    $set_fragile_while = "UPDATE `$room` SET `coordsW` = `coordsW` + '$moveFragileW', `coordsD` = `coordsD` + '$moveFragileD', `coordsH` = `coordsH`, `tick` = `tick` + 1, `play` = 1, `seek` = '0.01', `minusPlusW` = '$mpMoveW', `mpChange` = '$mpChange' WHERE `whatIsThis` = '$id';"; 
    } else {
        $set_fragile_while = "UPDATE `$room` SET `coordsW` = `coordsW` + '$moveFragileW', `coordsD` = `coordsD` + '$moveFragileD', `coordsH` = `coordsH`, `tick` = `tick` + 1, `minusPlusW` = '$mpMoveW', `mpChange` = '$mpChange' WHERE `whatIsThis` = '$id';"; 
    }
    }
    ...
    */
    ...
    if ($i > 79) { 
        exec('php ' . $fleoPathAbs . '/fleo.at_1.0.0/fleo.at-php/fragiles/peopleGoingForAWalk.php 1 '.$id.' '.$room.' > /dev/null &');
        $goBaby = 0;
    }
    if ($row["program"] !== $meIam) {
        $goBaby = 0;
