...
$get_fragile = $fleo_pdo->prepare("SELECT `program` FROM `$room` WHERE `id` = '1246';");
...
$set_fragile = "UPDATE `$room` SET `program` = '$meIam' WHERE `id` = '1246';";
$fleo_pdo->exec($set_fragile);

$i = $intensity * 5; // intensity = 5 // i = 10
$iH = $intensity; // iH = 10
while ($i > 0) {

$strength = (($intensity)) / ($intensity * 2 / $i);
...
$moveFragileH = intval($iH * $fieldH * 20);
   ...
   $iH -= 2;
    ...
    $set_fragile_while = "UPDATE `room-home` SET `coordsW` = `coordsW` + '$moveFragileW', `coordsD` = `coordsD` + '$moveFragileD', `coordsH` = GREATEST(0, coordsH + $moveFragileH), `tick` = `tick` + 1 WHERE `id` = '1246' AND `program` = '$meIam';";
    ...
    if ($i == 0) { 
        ...
        usleep(125000);
        exec('php ' . $fleoPathAbs . '/fleo.at_1.0.0/fleo.at-php/fragiles/basketbol.php 1 '.$field.' '.$sendIntensity.' '.$view.' > /dev/null &');
    }
    }
    echo "done?", PHP_EOL;
    }
