...
if ($.cookie('spacename')) {
    localStorage.setItem('spacename', $.cookie('spacename'));
    Cookies.set('spacename', 'empty', {
        secure: true,
        sameSite: 'none',
        expires: -1
    });
...
if ($.cookie('spacecolor')) {
    localStorage.setItem('spacecolor', $.cookie('spacecolor'));
    Cookies.set('spacecolor', 'empty', {
        secure: true,
        sameSite: 'none',
        expires: -1
    });
}
if ($.cookie('spacecoords')) {
    localStorage.setItem('spacecoords', $.cookie('spacecoords'));
    Cookies.set('spacecoords', 'empty', {
        secure: true,
        sameSite: 'none',
        expires: -1
    });
}
if ($.cookie('spacedoords')) {
    localStorage.setItem('spacedoords', $.cookie('spacedoords'));
    Cookies.set('spacedoords', 'empty', {
        secure: true,
        sameSite: 'none',
        expires: -1
    });
    console.log("Cookies replaced with localStorage");
}
...
function audioStarted() {
    ...
    Push.config({
        serviceWorker: '/fleo.at-js/jquery-ui/serviceWorker.min.js'
    });
            ...
            async function doTheVolumeMeterOwn(stream) {
                ...
                volumeMeterNode["callerAudio-Own"].port.onmessage = event => {
                    ...
                    if (event.data.volume) {
                        ...
                        ledsOwn((_volume * 100) / _sensibility)
                    }
                }
                volumeMeterGrayNode["callerAudio-Own"].port.onmessage = event => {
                    ...
                    if (event.data.volume) {
                        _volume = event.data.volume;
                        ledsOwnGray((_volume * 100) / _sensibility)
                    }
                }
    ...
    easyrtc.setStreamAcceptor(function (easyrtcid, stream) {
        ...
        async function doTheVolumeMeterOthers(streamMod) {
            ...
            volumeMeterNode["callerAudio-" + easyrtcid].port.onmessage = event => {
                ...
                if (event.data.volume) {
                    _volume = event.data.volume;
                    ledsRec((_volume * 100) / _sensibility)
                }
            }
            volumeMeterGrayNode["callerAudio-" + easyrtcid].port.onmessage = event => {
                ...
                if (event.data.volume) {
                    _volume = event.data.volume;
                    ledsRecGray((_volume * 100) / _sensibility)
                }
            }
    ...
    easyrtc.setOnStreamClosed(function (easyrtcid) {
        ...
        $(".maennikenConferenceHangup-" + calleeActive3).hide();
        $(".maennikenConferenceVolume-" + calleeActive3).hide();
        ...
        setTimeout(function () {
            if (!$(".endCallHangUp:visible").length) {
                $("#hangUpButton2").hide();
            }
        }, 1500);
    });
    ...
    easyrtc.setAcceptChecker(function (easyrtcid, callback) {
        ...
        if (!window.focus()) {
            Push.create("Incoming Call!", {
                body: "You are called by " + whoCalls,
                icon: '/icon.png',
        ...
        ringringTimes = 0;
        ringring.volume = 0.2;
        var ringringTimOut = setTimeout(function () {
            ringring.play();
        }, 2500);
        var ringringIntVal = setInterval(function () {
            if (ringringTimes < 8) {
                ringring.play();
                ringringTimes++;
            } else {
        ...
        var acceptTheCall = function (wasAccepted) {
            ...
            if (wasAccepted) {
                $("#accept-" + easyrtcid).remove();
                $("#reject-" + easyrtcid).remove();
        ...
        document.getElementById("accept-" + easyrtcid).onclick = function () {
            ...
            $(".maennikenConferenceHangup-" + calleeActive2).show();
            $(".maennikenConferenceVolume-" + calleeActive2).show();
            $("#hangUpButton2").show();
        ...
        toastr.info("You are called by " + whoCalls, mainDomain, {
            onclick: function () {
                ...
                if (calleeActive !== (myNumber[0] + myNumber[2]).replace("#", "")) {
                    ...
                    easyrtc.call(
                        ...
                        function success(otherCaller) {
                            ...
                            $(".maennikenConferenceHangup-" + calleeActive).show();
                            $(".maennikenConferenceVolume-" + calleeActive).show();
                            $("#hangUpButton2").show();
    ...
    function addToConversation(who, msgType, content) {
        ...
        if (!window.focus()) {
            Push.create("Chat!", {
                body: $("#maennikenTextWall-" + chatPartner).html() + ": " + content,
                requireInteraction: true,
                icon: '/icon.png',
                tag: "incomingChat",
                onClick: function () {
    ...
    sendStuffWS = function (otherEasyrtcid) {
        ...
        if (typeof $("#maennikenTextWall-" + chatPartner).html() == "undefined") {
            text = "Konnte nicht senden. Empfänger scheint fort.";
        }
...
function getThingsStraight() {
    if (yeahStartup !== 1) {
        $("html").animate({
            ...
            w = $(window).width(), h = $(window).height(), thisAllScale = w / 1800, $("#wrapperContainer").transition({
                scale: thisAllScale,
                duration: 0
            }, (function () {
...
$("#wrapper").bind('mousewheel DOMMouseScroll', function (e) {
    ...
    if (scrollTo) {
        e.preventDefault();
        scrollSub += scrollTo;
        if (scrollSub > 50 || scrollSub < -50) {
            relationToBackground = $(window).scrollTop() + scrollSub - parseInt($("body").css("height")) + $(window).height();
            scrollSub = 0;
...
function changeVolumeMeMoving(otherPersonCode) {
    if ($(".maennikenConferenceHangup-" + otherPersonCode).is(":visible")) {
        ...
        if (othPersonDistanceSelf[otherPersonCode] > 15000) {
            othPersonDistanceSelfId[otherPersonCode] = 0;
        } else {
            othPersonDistanceSelfId[otherPersonCode] = Math.round(90 + (othPersonDistanceSelf[otherPersonCode] / 13500) * -96);
            if (hearLikeOwl == 1) {
                othPersonDistanceSelfId[otherPersonCode] = 84
            }
            if (othPersonDistanceSelfId[otherPersonCode] < 0) {
                othPersonDistanceSelfId[otherPersonCode] = 0;
            }
            if (othPersonDistanceSelfId[otherPersonCode] > 84) {
                othPersonDistanceSelfId[otherPersonCode] = 84;
            }
...
function whereAmI(hiCo, sMMs, level, duration, turn, whoC, colorC) {
    ...
    $(".audioStation").each(function () {
        ...
        if (sideVolume[$(this)] < doordsVolume[$(this)]) {
            setVolume[$(this)] = sideVolume[$(this)];
        } else {
            setVolume[$(this)] = doordsVolume[$(this)];
        }
        if (setVolume[$(this)] < 0) {
            setVolume[$(this)] = 0;
        }
    ...
    if (will == "b") {
        $("#mapForLocator").css({
            "transform-origin": mySpotterMapLocationX + "px " + mySpotterMapLocationY + "px",
            "bottom": mySpotterMapLocationY - 200 + "px",
            "left": w / 2 - mySpotterMapLocationX + "px"
        }).transition({
            perspective: '100px',
            rotate: (turn / 9000) * 90 + 'deg'
        }, {
            duration: 0
        });
    } else {
        $("#mapForLocator").css({
            ...
            perspective: '100px',
            rotate: (turn / 9000) * -90 + 180 + 'deg'
        }, {
            duration: 0
        });
    }
    $("#maennikenSpotter-Own").css({
        "top": mySpotterMapLocationY - 5 + "px",
        "left": mySpotterMapLocationX - 5 + "px"
    });
...
$("#hideExplanationDiv").click(function () {
    $(".explanation").remove();
    $("#hideExplanationDiv").remove();
...
for (let i = 1; i < 23; i++) {
    for (let j = 1; j < 46; j++) {
        $("#mapForLocatorClickDivs").append('<div clickcoords="' + j + '" clickdoords="' + i + '" class="locClick"></div>');
    }
}
...
if (Device_Type() == "Mobile" || Device_Type() == "Tablet") {
    ...
    $("#mapForLocatorClickDivs").removeClass("locClickHover");
}
...
$("#sendMessageButton").click(function () {
    sendStuffWS($("#writeChat").attr("receiver"));
    $("#writeChat").hide();
});
...
$("#menuBottom").append('<div id="dropBuilding" class="cockpit menuItem" style="display:none;">Drop building</div>');
...
$("#manual").click(function () {
    if (manualBuilt == 0) {
        ...
        $("#sendMathQuestion").click(function () {
            $.post("/fleo.at-php/fleo.at_bag.php", {
                doing: 9,
                iam: (myNumber[0] + myNumber[2]).replace("#", ""),
                result: $("#mathQuestion").val()
...
if (window.name == "tCupSpaceOpenInOtherWindow") {
    if (!window.toolbar.visible || !window.menu.visible) {
        $("#niceWindowToggle").hide();
    }
}
...
$("#fleoDrawGet").click(function () {
    if (drawBuilt == 0) {
        ...
        $.get("/fleo.at-php/fleo.at_drawCage.php", function (data) {
            $("#wrapper").append(data);
        });
        drawOn = 1;
    } else {
...
$("#sendPresent").click(function () {
    ...
    $("#sendAnythingButton").click(function () {
        spacebarText = 0;
        $.post("/fleo.at-php/fleo.at_setOfflineOnline.php", {
            doing: 2,
            fleoNumOwn: (myNumber[0] + myNumber[2]).replace("#", ""),
            fleoNumDistant: fleoNumDistant,
            extraContent: document.getElementById('sendAnythingText').value
        }).done(function (result) {
            toastr.success("Trying to give it to: " + $("#maennikenTextWall-" + fleoNumDistant).html());
        });
        $("#writeAnythingTo").hide();
    });
    $("#sendAnythingCancel").click(function () {
        spacebarText = 0;
        $("#writeAnythingTo").hide();
    });
...
$("#haeuserf").append('<div style="position:absolute;left:0;bottom:60px;z-index:0;width:1800px;height:auto;"><img src="https://' + mainDomain + '/fleo.at-medien/repertoire/haeuserf.webp" style="width:100%;height:auto;" /></div>');
...
$("#dropBuilding").click(function () { 

    if (dropBuildingMenu == 0) {
        dropBuildingMenu = 1;
        $("body").append('<div id="dropBuildingMenu" style="color:black;"><h1 style="background:#ffffff8f;">Drop any building</h1><h2 style="background:#ffffff8f;">Choose and adjust.</h2><p style="background:#ffffff8f;"><input id="mathQuestion" minlength="1" maxlength="2" size="2"> <span id="mathQuestionResult"><button id="sendMathQuestion">Senden</button></span></p>');
        $(".dropBuildingButton").click(function () {
            /* $.post("/fleo.at-php/fleo.at_bag.php", {
                doing: 9,
                iam: (myNumber[0] + myNumber[2]).replace("#", ""),
                result: $("#mathQuestion").val()
        ...
        $("#dropBuildingMenu").dialog({
            title: "Drop Building Menu",
            autoOpen: false,
            minWidth: 600,
            modal: true,
            buttons: {
    ...
    if ($("#dropBuildingMenu").dialog("isOpen")) {
        $("#dropBuildingMenu").dialog("close");
    } else {
        $("#dropBuildingMenu").dialog("open");
    }
...
$("#infot").click(function () {
    ...
    $(".person").each(function () {
        $("#maennikenBag-" + $(this).find(".tree").attr("id").replace("personTree-", "")).css({
            "left": 82 - parseInt(($("#maennikenBag-" + $(this).find(".tree").attr("id").replace("personTree-", "")).width() / 2)) + "px",
            "bottom": 90 - parseInt(($("#maennikenBag-" + $(this).find(".tree").attr("id").replace("personTree-", "")).height() / 2)) + "px"
        });
    });
});
...
function move(coordsTr, doordsTr, duration, height) {
    ...
    $(".maennikenLocator").each(function () {
        ...
        if (locateVertical < 150) {
            $("#" + locatorToMove).css("background", "red");
            bottomLocate = 150 + (150 - Math.abs(locateVertical)) + "px";
        } else {
...
function roomChange(newRoom) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "/fleo.at-php/fleo.at_roomChange.php",
        dataType: "html",
        data: {
            doing: 1,
            number: (myNumber[0] + myNumber[2]).replace("#", ""),
        ...
        setTimeout(function () {
            $.ajax({
                ...
                for (var p = 0; p < ppp.length; p++) {
                    if ($(ppp.item(p)).attr("room") == myRoom) {
                        $(ppp.item(p)).removeClass("otherroom");
                    } else {
                        $(ppp.item(p)).addClass("otherroom");
                    }
                }
                $("#haeuserf").css({
                    "left": 0
...
$(document).keydown(function (event) {
    ...
    if (spacebarText == 0) {
        ...
        if (event.which == 38) { // forward
            ...
            if (keys == 1) {
                if (will == "b") {
                    moveWithTrain(historyCoords + (turn / 2), Math.round(sMMssM - ((1 - (Math.abs(turn) / 9000)) * 300)), 0, relationToBackground, turn);
                } else {
                    moveWithTrain(historyCoords + (turn / 2), Math.round(sMMssM + ((1 - (Math.abs(turn) / 9000)) * 300)), 0, relationToBackground, turn);
                }
            }
...
$(".chtop").click(function (event) {
    event.preventDefault();
    if (will == "b") {
        moveWithTrain(historyCoords, sMMssM - 300, 0, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords, sMMssM + 300, 0, relationToBackground, turn);
    }
});
...
$(".chleft").click(function (event) {
    event.preventDefault();
    if (will == "b") {
        moveWithTrain(historyCoords + 300, sMMssM, 0, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords - 300, sMMssM, 0, relationToBackground, turn);
    }
});
...
$("#navFor").click(function () {
    if (will == "b") {
        moveWithTrain(historyCoords + (turn / 2) * 1, Math.round(sMMssM - ((1 - (Math.abs(turn) / 9000 / 1)) * 300)), 0, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords + (turn / 2) * 1, Math.round(sMMssM + (1 - (Math.abs(turn) / 9000))), 0, relationToBackground, turn);
    }
});
$("#navForStrong").click(function () {
    if (will == "b") {
        moveWithTrain(historyCoords + (turn / 2) * 3, Math.round(sMMssM - ((3 - (Math.abs(turn) / 9000 / 3)) * 300)), 0, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords + (turn / 2) * 3, Math.round(sMMssM + (3 - (Math.abs(turn) / (9000 / 3)))), 0, relationToBackground, turn);
    }
});
$("#navForWheel").click(function () {
    if (will == "b") {
        moveWithTrain(historyCoords + (turn / 2) * 7, Math.round(sMMssM - ((7 - (Math.abs(turn) / 9000 / 7)) * 300)), 0, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords + (turn / 2) * 7, Math.round(sMMssM + (7 - (Math.abs(turn) / (9000 / 7)))), 0, relationToBackground, turn);
    }
});
...
$("#navBac").click(function () {
    if (will == "b") {
        moveWithTrain(historyCoords - (turn / 2) * 1, Math.round(sMMssM + ((3 - (Math.abs(turn) / 9000 / 1)) * 300)), 0, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords + (turn / 2) * -1, Math.round(sMMssM - (1 - (Math.abs(turn) / 9000))), 0, relationToBackground, turn);
    }
});
...
$("#navLef").click(function () {
    if (will == "b") {
        moveWithTrain(historyCoords + 300, sMMssM, 250, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords - 300, sMMssM, 250, relationToBackground, turn);
    }
});
$("#navLefStrong").click(function () {
    if (will == "b") {
        moveWithTrain(historyCoords + 700, sMMssM, 250, relationToBackground, turn);
    } else {
        moveWithTrain(historyCoords - 700, sMMssM, 250, relationToBackground, turn);
    }
});
...
$(".chtopleft").click(function () {
    if (will == "b") {
        if (turn + 200 < 9000 && turn + 200 > -9000) {
            ...
            $.post("/fleo.at-php/fleo.at_turn.php", {
                doing: 1,
                turn: turn,
                fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
            });
...
$(".chleft").on('mousewheel', function (event) {
    event.preventDefault();
    if (will == "b") {
        moveWithTrain(parseInt(historyCoords + ((1 - (Math.abs(turn) / 9000)) * event.deltaFactor * event.deltaY * 3)),
...
$(".chleft").swipe({
    swipe: function (event, direction, distance, duration, fingerCount, fingerData) {
        ...
        if (will == "b") {
            moveWithTrain(parseInt(historyCoords + ((1 - (Math.abs(turn) / 9000)) * distance * 3)),
...
$(".chleft").on('click', function (event) {
    if (will == "b") {
        moveWithTrain(parseInt(historyCoords + Math.round((300) * 2)), sMMssM, 0, relationToBackground, turn);
    } else {
        moveWithTrain(parseInt(historyCoords + Math.round((-300) * 2)), sMMssM, 0, relationToBackground, turn);
    }
    event.preventDefault();
...
let startX, startY, centerX, centerY, deltaXStore = 0,
    deltaYStore = 0,
    currentXStore = 0,
    currentYStore = 0;
var currentlyInput = 1,
    kindOfWheel = 2;
...
const updatePosition = (currentX, currentY) => {
    ...
    if (will == "b") {
        ...
        $(".move > .tree").css({
            "left": "-=" + parseInt(historyCoords - hiCo)
        });
...
var mapLatitude, mapLongitude, mapLocationX, mapLocationY;
...
$("#taxi-send").click(function () {
    if (!$("#taxi-coords").val()) {
        ...
        taxiCoords = parseInt($("#taxi-coords").val());
    }
    if (!$("#taxi-doords").val()) {
        taxiDoords = 0;
    } else {
        taxiDoords = parseInt($("#taxi-doords").val());
    }

    moveWithTrain(taxiCoords, taxiDoords * 3, 2000, relationToBackground, turn);
    $("#fixedLocation").dialog("close");
    $("#TrainWOMapDiv").show();
    $("#mapDiv").hide();
});
$(".goNothingLink").click(function () {
    $("#fixedLocation").dialog("close");
    $("#TrainWOMapDiv").show();
    $("#mapDiv").hide();
})

if (urlForCoordsGet.get('location') == "go") {
    $("#fixedLocation").dialog("close");
};
$("#fixedLocationOpen").click(function () {
    $("#fixedLocation").dialog("open");
    $("#fixedLocationOpen").fadeOut();
});

var canvasTrain = document.getElementById("mapForTrain"),
    ctx = canvasTrain.getContext("2d");
...
$("#goMap").click(function () {
    ...
    if (mapLatitude < -180) {
        mapLatitude = -180;
    } else if (mapLatitude > 180) {
        mapLatitude = 180;
    };
    if (mapLongitude < -90) {
        mapLongitude = -90;
    } else if (mapLongitude > 90) {
        mapLongitude = 90;
    };
...
function connectWorld() {
    ...
    worldSrc.addEventListener('lettercoins', function (e) {
        ...
        if (haltEverything == 0) {
            ...
            if (doLaterLCLettercoins) {
                ...
                lettercoins[doLaterLCWhatisThis] = [];
            }
            if (lettercoins[doLaterLCWhatisThis][1]) {
                $("#letterCoinMillAudioStationTextLettercoin-1-" + doLaterLCWhatisThis).html(lettercoins[doLaterLCWhatisThis][1]);
            } else {
                $("#letterCoinMillAudioStationTextLettercoin-1-" + doLaterLCWhatisThis).html("");
            }
            if (lettercoins[doLaterLCWhatisThis][2]) {
                $("#letterCoinMillAudioStationTextLettercoin-2-" + doLaterLCWhatisThis).html(lettercoins[doLaterLCWhatisThis][2]);
            } else {
                $("#letterCoinMillAudioStationTextLettercoin-2-" + doLaterLCWhatisThis).html("");
            }
        } else {
    ...
    function reactToChanges() {
        ...
        var radioWebsite1 = "https://stream.radiofrance.fr/francemusiqueeasyclassique/francemusiqueeasyclassique_hifi.aac?id=" + mainDomain;
            ...
            if ($('[data-attr=' + worldNews2.id + ']').length) {
                ...
                if (isChangeW == 1 || isChangeD == 1 || isChangeH == 1 || isChangeS == 1) {
                    ...
                    if (isChangeW == 1) {
                        ...
                        if (worldNews2.minusPlusW == "0") {
                            if (oldDirectionMoveW[worldNews2.id] !== "0") {
                                $("[data-attr='" + worldNews2.id + "']").find(".tree").css({
                                    "transition": "transform 4.0s linear 0s",
                        ...
                        if (worldNews2.minusPlusW == "1") {
                            if (oldDirectionMoveW[worldNews2.id] !== "1") {
                                $("[data-attr='" + worldNews2.id + "']").find(".tree").css({
                                    "transition": "transform 4.0s linear 0s",
                                    "transform-origin": "center",
                                    "transform": "scaleX(-1)"
                ...
                if (worldNews2.tick2 - worldNews2.tick == 2) {
                    ...
                    if (am == "f") {
                        if (worldNews2.whatIsThis.includes("audStat")) {
                            ...
                            if (worldNews2.floor == "on") {
                                $("#wrapper").append('<div class="move floor scale' + (worldNews2.coordsD + sMMssM) + '" distance="' + (worldNews2.coordsD + sMMssM) + '" playing="' + worldNews2.play + '" data-attr="' + worldNews2.id + '" data-online="' + worldNews2.isOnline + '"><div ' + worldNews2.object + ' width: ' + worldNews2.width + 'px; bottom: ' + worldNews2.coordsH + 'px; left: ' + (worldNews2.coordsW + historyCoords) + 'px;">' + stripslashes(htmlDecode(worldNews2.name)) + '</div></div><script>' + stripslashes(htmlDecode(worldNews2.script)) + '</script>');
                            } else {
                                $("#wrapper").append('<div class="move thing scale' + (worldNews2.coordsD + sMMssM) + '" distance="' + (worldNews2.coordsD + sMMssM) + '" playing="' + worldNews2.play + '" data-attr="' + worldNews2.id + '" data-online="' + worldNews2.isOnline + '"><div ' + worldNews2.object + ' width: ' + worldNews2.width + 'px; bottom: ' + worldNews2.coordsH + 'px; left: ' + (worldNews2.coordsW + historyCoords) + 'px;">' + stripslashes(htmlDecode(worldNews2.name)) + '</div><script>' + stripslashes(htmlDecode(worldNews2.script)) + '</script></div>');
                            }
                        }
                    } else if (am == "b") {
                        if (worldNews2.whatIsThis.includes("audStat")) {
                            ...
                            if (worldNews2.floor == "on") {
                                $("#wrapper").append('<div class="move floor scale' + ((worldNews2.coordsD * -1) - sMMssM) + '" distance="' + ((worldNews2.coordsD * -1) - sMMssM) + '" playing="' + worldNews2.play + '" data-attr="' + worldNews2.id + '" data-online="' + worldNews2.isOnline + '"><div ' + worldNews2.object + ' width: ' + worldNews2.width + 'px; bottom: ' + worldNews2.coordsH + 'px; left: ' + (worldNews2.coordsW + historyCoords - worldNews2.width) + 'px;">' + stripslashes(htmlDecode(worldNews2.name)) + '</div></div><script>' + stripslashes(htmlDecode(worldNews2.script)) + '</script>');
                            } else {
                                $("#wrapper").append('<div class="move thing scale' + ((worldNews2.coordsD * -1) - sMMssM) + '" distance="' + ((worldNews2.coordsD * -1) - sMMssM) + '" playing="' + worldNews2.play + '" data-attr="' + worldNews2.id + '" data-online="' + worldNews2.isOnline + '"><div ' + worldNews2.object + ' width: ' + worldNews2.width + 'px; bottom: ' + worldNews2.coordsH + 'px; left: ' + (worldNews2.coordsW + historyCoords - worldNews2.width) + 'px;">' + stripslashes(htmlDecode(worldNews2.name)) + '</div><script>' + stripslashes(htmlDecode(worldNews2.script)) + '</script></div>');
                            }
                        }
                    }
            ...
            if ($('[data-attr=' + worldNews2.id + ']').length) {
                ...
                if (haltEverything == 0) {
                    if (worldNews2.play == 1 && worldNews2.seek == 0 && worldNews2.whatIsThis.includes("audStat") || worldNews2.play == 1 && playerPlayingOld[worldNews2.whatIsThis] !== worldNews2.play && worldNews2.whatIsThis.includes("audStat")) {
                        ...
                        if (worldNews2.whatIsThis == "audStat-1738704492B1-Portis") { 
                            document.getElementById("audStat-1738704492B1-Portis").src = radioWebsite1; document.getElementById(worldNews2.whatIsThis).currentTime = 0; document.getElementById(worldNews2.whatIsThis).play(); } else
                ...
                if (isChangeW == 1 || isChangeD == 1 || isChangeH == 1 || isChangeS == 1) {
                    ...
                    if (worldNews2.whatIsThis.includes("audStat") || worldNews2.whatIsThis == "ytplayer") {
                        ...
                        if (sideVolume[$("#" + worldNews2.whatIsThis)] < doordsVolume[$("#" + worldNews2.whatIsThis)]) {
                            setVolume[$("#" + worldNews2.whatIsThis)] = sideVolume[$("#" + worldNews2.whatIsThis)];
                        } else {
                            setVolume[$("#" + worldNews2.whatIsThis)] = doordsVolume[$("#" + worldNews2.whatIsThis)];
                        }
                        if (setVolume[$("#" + worldNews2.whatIsThis)] < 0) {
                            setVolume[$("#" + worldNews2.whatIsThis)] = 0;
                        }
        ...
        if (genesisDone == 0) {
            ...
            if (!$('[data-attr=' + worldNews.id + ']').length) {
                ...
                if (worldNews.whatIsThis.includes("audStat")) {
                    ...
                    if (worldNews.floor == "on") {
                        world2GeneratePortion[world2GeneratePortionCount] += '<div class="move floor scale' + worldNews.coordsD + '" distance="' + worldNews.coordsD + '" playing="' + worldNews.play + '" data-attr="' + worldNews.id + '" data-online="' + worldNews.isOnline + '"><div ' + worldNews.object + ' width: ' + worldNews.width + 'px; bottom: ' + worldNews.coordsH + 'px; left: ' + worldNews.coordsW + 'px;">' + stripslashes(htmlDecode(worldNews.name)) + '</div></div><script>' + stripslashes(htmlDecode(worldNews.script)) + '</script>';
                    } else {
    ...
    $("#wrapper").arrive(".audioStation", function () {
        ...
        if (audFunk[audioStationToAssign] == null) {
            audFunk[audioStationToAssign] = function () {};
        }
        ...
        if ($(this).is("video")) {
            ...
            document.getElementById(audioStationToAssign).onplay = function () {
                ...
                $(".audioStationAudioPlay-" + audioStationToAssign).hide();
                $(".audioStationAudioStop-" + audioStationToAssign).show();
            ...
            document.getElementById(audioStationToAssign).onended = function () {
                ...
                $("#letterCoinMillAudioStationTextLettercoin-1-" + audioStationToAssign).effect("pulsate", {
                    ...
                    $("#letterCoinMillAudioStationTextLettercoin-1-" + audioStationToAssign).css({
                        "pointer-events": "auto",
                        "background": "green"
                    });
                });
                ...
                $("#audioStationAudioDiv-" + audioStationToAssign).animate({
                    "bottom": "350px"
                }, 1000);
                ...
                if ($(this).attr("id") == "audStat-16713162473-01Mete" || $(this).attr("id") == "audStat45614724") {
                    document.getElementById(worldNews2.whatIsThis).src = "";
                }
            };
            ...
            $(".audioStationAudioStop-" + audioStationToAssign).click(function () {
                if ($("#videoPlayerControlsOuter-" + audioStationToAssign).hasClass("startedSimple")) {
                    ...
                    $.post("https://" + mainDomain + "/fleo.at-php/fleo.at_timeSeek.php", {
                        ...
                        room: myRoom,
                        audioStation: audioStationToAssign,
                        seek: document.getElementById(audioStationToAssign).currentTime
                    });
            ...
            document.getElementById(audioStationToAssign).oncanplaythrough = function () {
                $("#videoPlayerInnerSeekBar-" + audioStationToAssign).slider({
                    ...
                    create: function () {
                        $("#videoPlayerControlsOuter-" + audioStationToAssign).mouseover(function () {
                            $("#videoPlayerControlsInner-" + audioStationToAssign).show();
                        });
                        $(".audioStationAudioStopLargeSimple-" + audioStationToAssign).mouseleave(function () {
                            if (!$("#videoPlayerControlsInner-" + audioStationToAssign)[0].matches('.seeking')) {
                                $("#videoPlayerControlsInner-" + audioStationToAssign).hide();
                            }
                        });
                        $(".audioStationAudioStopLargeAll-" + audioStationToAssign).mouseleave(function () {
                            if (!$("#videoPlayerControlsInner-" + audioStationToAssign)[0].matches('.seeking')) {
                                $("#videoPlayerControlsInner-" + audioStationToAssign).hide();
                            }
                        });
                        $("#videoPlayerInnerSeekBar-" + audioStationToAssign).mouseleave(function () {
                            if ($(".audioStationAudioStopLargeSimple-" + audioStationToAssign).is(":visible")) {
                                if (!$("#videoPlayerControlsInner-" + audioStationToAssign)[0].matches('.seeking')) {
                                    $("#videoPlayerControlsInner-" + audioStationToAssign).hide();
                                }
                            }
                        });
                        $("#videoPlayerControlsOuter-" + audioStationToAssign).mouseleave(function () {
                            if (!$("#videoPlayerControlsInner-" + audioStationToAssign)[0].matches('.seeking')) {
                                $("#videoPlayerControlsInner-" + audioStationToAssign).hide();
                            }
                        });
...
function socketDataConnect() {
    ...
    $.post("/fleo.at-php/fleo.at_sensorsStart.php", {
        doing: 1,
        fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
    });
    ...
    var myGeoLat = "noLatYet";
    var myGeoLon = "noLonYet";
    ...
    function geoSuccess(position) {
        ...
        myGeoLon = position.coords.longitude;
        ...
        if (locationBound == 1) {
            moveWithTrain(Math.trunc(position.coords.longitude * -1000), Math.trunc(position.coords.latitude * -300), 200, 0, 0);
        }
    }

    function geoError() {
        $("#unbindLocation").css("background", "#ff0000").animate({
            "background-color": "rgb(214, 214, 195)"
        }, 5000);
    }
    ...
    function sensorsWall() {
        setTimeout(function () {
            $("body").append('<div id="sensorsWall" style="pointer-events:none;position:fixed;width:300px;height:420px;bottom:0;right:0;background:#ffffff4f;color:#ffffff7f;display:none;"></div>');
            ...
            function geoSuccess(position) {
                ...
                if (wsOk == 1 && isWindowTop == 1) {
                    $("#sensorsHeading").html(position.coords.heading);
                }
                ws.send(JSON.stringify({
                    type: "geoHeading",
            ...
            var deviceOrientation = FULLTILT.getDeviceOrientation({
                'type': 'world'
            });
            ...
            deviceOrientation.then(function (orientationData) {
                ...
                orientationData.listen(function () {
                    ...
                    if (wsOk == 1 && isWindowTop == 1) {
                        $("#orientationAlpha").html(rawEvent.alpha);
                    }
                    ws.send(JSON.stringify({
                        type: "orientationAlpha",
                        value: rawEvent.alpha,
                        fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                    }));
                    if (wsOk == 1 && isWindowTop == 1) {
                        $("#orientationBeta").html(rawEvent.beta);
                    }
                    ws.send(JSON.stringify({
                        type: "orientationBeta",
                        value: rawEvent.beta,
                        fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                    }));
                    if (wsOk == 1 && isWindowTop == 1) {
                        $("#orientationGamma").html(rawEvent.gamma);
                    }
                    ws.send(JSON.stringify({
                        type: "orientationGamma",
                        value: rawEvent.gamma,
                        fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                    }));
                    ws.send(JSON.stringify({
                        type: "orientationCompass",
                        value: rawEvent.absolute,
                        fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                    }));
                    if (wsOk == 1 && isWindowTop == 1) {
                        $("#webkitCompass").html(rawEvent.beta);
                    }
                    ws.send(JSON.stringify({
                        type: "webkitCompass",
                        value: rawEvent.beta,
                    ...
                    if (wsOk == 1 && isWindowTop == 1) {
                        $("#webkitCompassAccuracy").html(rawEvent.webkitCompassAccuracy);
                    }
                    ws.send(JSON.stringify({
                        type: "webkitCompassAccuracy",
                        value: rawEvent.webkitCompassAccuracy,
                        fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                    }));
                });
            });
            ...
            try {
                ...
                if (gravitySensor) {
                    gravitySensor.addEventListener("reading", (e) => {
                        if (wsOk == 1 && isWindowTop == 1) {
                            $("#sensorsGrax").html(gravitySensor.x);
                        }
                        ws.send(JSON.stringify({
                            type: "graX",
                            value: gravitySensor.x,
                            fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                        }));
                        if (wsOk == 1 && isWindowTop == 1) {
                            $("#sensorsGray").html(gravitySensor.y);
                        }
                        ws.send(JSON.stringify({
                            type: "graY",
                            value: gravitySensor.y,
                            fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                        }));
                        if (wsOk == 1 && isWindowTop == 1) {
                            $("#sensorsGraz").html(gravitySensor.z);
                        }
                        ws.send(JSON.stringify({
                            type: "graZ",
                            value: gravitySensor.z,
                            fleoNum: (myNumber[0] + myNumber[2]).replace("#", "")
                        }));
                    });
                    gravitySensor.start();
            ...
            let laSensor = null;
            try {
                laSensor = new LinearAccelerationSensor({
                    frequency: 10
                });
                if (laSensor) {
                    laSensor.addEventListener('reading', (e) => {
                        if (wsOk == 1 && isWindowTop == 1) {
                            $("#sensorsAccLinx").html(laSensor.x);
                        }
                        ws.send(JSON.stringify({
                            type: "accLX",
                            value: laSensor.x,
                        ...
                        if (wsOk == 1 && isWindowTop == 1) {
                            $("#sensorsAccLiny").html(laSensor.y);
                        }
                        ws.send(JSON.stringify({
                            type: "accLY",
                            value: laSensor.y,
                        ...
                        if (wsOk == 1 && isWindowTop == 1) {
                            $("#sensorsAccLinz").html(laSensor.z);
                        }
                        ws.send(JSON.stringify({
                            type: "accLZ",
                            value: laSensor.z,
        ...
        try {
            if ("AmbientLightSensor" in window) {
                ...
                sensorAmb.addEventListener("reading", (event) => {
                    if (wsOk == 1 && isWindowTop == 1) {
                        $("#sensorsAmb").html(sensor.illuminance);
                    }
                    ws.send(JSON.stringify({
                        type: "ambL",
...
$("body").arrive("#push2TalkUser", function () {
    ...
    $("#push2TalkOwnMute").click(function () {
        document.getElementById("callerAudio-Own").volume = 0;
        $("#push2TalkOwnMute").toggle();
        $("#push2TalkOwnUnmute").toggle();
    });
    $("#push2TalkOwnUnmute").click(function () {
        document.getElementById("callerAudio-Own").volume = 1;
        $("#push2TalkOwnUnmute").toggle();
        $("#push2TalkOwnMute").toggle();
    });
    ...
    setTimeout(function () {
        ...
        mediaRecorderArray[mediaRecorderArray.length - 1].content.onstop = function (e) {
            ...
            for (let i = 0; i < mediaRecorderArray.length; i++) {
                blobToRec[i] = new Blob(chunks[i], {
                    'type': 'audio/webm; codecs=opus'
                });
                if (blobToRec[i].size > 0) {
                    streamsNum++;
                    if (!recReader[i]) {
                        recReader[i] = new FileReader();
                    }
                        ...
                        if (replayAndRecordBuilt == 0) {
                            ...
                            $(document).on("click", "#push2TalkReplayRec", function (e) {
                                if (doReplayAndRecord == 1) {
                                    $("#push2TalkReplayRec").css("text-decoration", "line-through");
                                    doReplayAndRecord = 0;
                                } else {
                                    $("#push2TalkReplayRec").css("text-decoration", "none");
                                    doReplayAndRecord = 1;
                                }
                            });
    ...
    fleoRecord.onclick = function () {
        ...
        if (replayAndRecordBuilt == 1) {
            ...
            if (doReplayAndRecord == 1) {
                ...
                if (audioSystemJustLoaded == 0 && recordingReset == 0) {
                    myAudioReplay.play().then(() => {
                        ...
                        async function doTheVolumeMeterReplayRec(streamReplay) {
                            ...
                            volumeMeterNode["callerAudio-Replay"].port.onmessage = event => {
                                ...
                                if (event.data.volume) {
                                    _volume = event.data.volume;
                                    ledsReplay((_volume * 100) / _sensibility)
                                }
                            }
                            volumeMeterGrayNode["callerAudio-Replay"].port.onmessage = event => {
                                ...
                                if (event.data.volume) {
                                    _volume = event.data.volume;
                                    ledsGrayReplay((_volume * 100) / _sensibility)
                                }
                            }
                ...
                for (let i = 0; i < mediaRecorderArray.length; i++) {
                    chunks[i] = [];
                    if (mediaRecorderArray[i].name == "callerAudio-Replay") {
                        delete mediaRecorder["callerAudio-Replay"];
                        delete mediaRecorderArray[i];
                    } else {
                        if (mediaRecorderArray[i].content.state == "inactive") {
                            mediaRecorderArray[i].content.start(5000);
        ...
        $("#push2TalkRec").toggle();
        $("#push2TalkRecUpload").toggle();
        $(".maennikenAudioPlay-rec").hide();
        $(".maennikenAudioStop-rec").hide();
        $(".maennikenAudioWhite-rec").show();
    }
    ...
    fleoStop.onclick = function () {
        ...
        for (let i = 0; i < mediaRecorderArray.length; i++) {
            ...
            if (mediaRecorderArray[i].content.state == "recording" || mediaRecorderArray[i].content.state == "paused") {
                mediaRecorderArray[i].content.stop();
            }
        }
    }
    mediaRecorderArray = [];
...
if (localStorage.getItem('spacenumber') || urlForCoordsGet.has('heronumber')) {
    ...
    if (location.pathname.replace("/r/", "") !== "/" && location.pathname.replace("/r/", "")) {
        startRoom = location.pathname.replace("/r/", "");
    } else {
        startRoom = "home";
    }
    myRoom = startRoom;
    ...
    $.ajax({
        ...
        window.history.replaceState("", "", "/r/" + startRoom);
...
$("#monoOrStereo").dialog({
    ...
    buttons: {
        ...
        "Ok, start": function () {
            if (location.pathname.replace("/r/", "") !== "/" && location.pathname.replace("/r/", "")) {
                roomWhereEdit = location.pathname.replace("/r/", "");
            }
            spacebarText = 1;
            $(this).dialog("close");
...
$("#push2TalkUserWants").click(function () {
    if (audioConferenceConnected == 0) {
        ...
        var waitForConnected = setInterval(function () {
            if (audioConferenceConnected == 1) {
                $("#push2TalkUserWants").hide();
                clearInterval(waitForConnected);
                if (location.pathname.replace("/r/", "") !== "/") {
                    roomWhereEdit = location.pathname.replace("/r/", "");
                }
                $("#thisBoxDialog").load("https://" + mainDomain + "/fleo.at-php/fleo.at_audioStationAdd.php?client=" + (myNumber[0] + myNumber[2]).replace("#", "") + "&buildcoords=" + (historyCoords - 500) + "&builddoords=" + (sMMssM - 2) + "&room=" + roomWhereEdit);
                $("#thisBoxDialog").dialog("open");
...
function doTheAudioStationStuff() {
    ...
    setTimeout(function () {
        $('input[name=buildnameAS]').attr("readonly", false);
    }, 5000);

    $('input[name=buildnameAS]').on('input', function (e) {
        $("#audioStationBuildText").html($('input[name=buildnameAS]').val())
    });
    ...
    function loadAudioSystem() {
        ...
        document.getElementById("callerAudio-ReplaySound").onplay = function () {
            $(".maennikenAudioWhite-rec").hide();
            $(".maennikenAudioPlay-rec").hide();
            $(".maennikenAudioStop-rec").show();
        };
        document.getElementById("callerAudio-ReplaySound").onended = function () {
            $(".maennikenAudioStop-rec").hide();
            $(".maennikenAudioPlay-rec").show();
        };
        document.getElementById("callerAudio-ReplaySound").onpause = function () {
            $(".maennikenAudioStop-rec").hide();
            $(".maennikenAudioPlay-rec").show();
        };
        document.getElementById("callerAudio-ReplayPlaySound").onplay = function () {
            $(".maennikenAudioWhite-rec").hide();
            $(".maennikenAudioPlay-rec").hide();
            $(".maennikenAudioStop-rec").show();
        };
        document.getElementById("callerAudio-ReplayPlaySound").onended = function () {
            $(".maennikenAudioStop-rec").hide();
            $(".maennikenAudioPlay-rec").show();
        };
        document.getElementById("callerAudio-ReplayPlaySound").onpause = function () {
            $(".maennikenAudioStop-rec").hide();
            $(".maennikenAudioPlay-rec").show();
        };
        document.getElementById("callerAudio-Own").onplay = function () {
            $(".maennikenAudioWhite-rec").hide();
            $(".maennikenAudioPlay-rec").hide();
            $(".maennikenAudioStop-rec").show();
        };
        document.getElementById("callerAudio-Own").onended = function () {
            $(".maennikenAudioStop-rec").hide();
            $(".maennikenAudioPlay-rec").show();
        };
        document.getElementById("callerAudio-Own").onpause = function () {
            $(".maennikenAudioStop-rec").hide();
            $(".maennikenAudioPlay-rec").show();
        };
        $(".maennikenAudioPlay-rec").click(function () {
            if (audioSystemJustLoaded == 0 && recordingReset == 0) {
                document.getElementById("callerAudio-Own").play();
            }
        });
        $(".maennikenAudioStop-rec").click(function () {
            document.getElementById("callerAudio-Own").pause();
            document.getElementById("callerAudio-ReplayPlaySound").pause();
            document.getElementById("callerAudio-ReplaySound").pause();
        });

        $("#push2Talk5s").click(function () {
            let tttt = 4;
            $("#push2TalkUser").click();
            for (let i = 0; i < 6; i++) {
                setTimeout(function () {
                    $("#push2Talk5s").html(tttt + "s");
                    $("#push2Talk5s").css("background", "white");
                    if (tttt > 0) {
                        $("#push2Talk5s").animate({
                            "background": "blue"
                        });
        ...
        $("#push2Talk10s").click(function () {
            let ttttt = 9;
            $("#push2TalkUser").click();
            for (let i = 0; i < 11; i++) {
                setTimeout(function () {
                    $("#push2Talk10s").html(ttttt + "s");
                    $("#push2Talk10s").css("background", "white");
                    if (ttttt > 0) {
                        $("#push2Talk10s").animate({
        ...
        $("#push2TalkRec").click(function () {
            $("#push2TalkUser").click();
        });
        $("#push2TalkRecUpload").click(function () {
            $("#push2TalkUserUpload").click();
        });
        ...
        if (audioStarted == 0) {
            $("#push2TalkUserCheck").slideDown();
            $("#push2Talk5s").slideDown();
            $("#push2Talk10s").slideDown();
            audioStarted = 1;
        }
    }
    loadAudioSystem();
    ...
    setTimeout(function () {
        if ($(window).width() < 800 || $(window).height() < 680) {
            $("#thisBoxDialog").dialog("option", "width", $(window).width());
            $("#thisBoxDialog").dialog("option", "height", $(window).height());
            $("#thisBoxDialog").dialog("option", "position", {
                my: "left top",
                at: "left top"
            });
        } else {
            $("#thisBoxDialog").dialog("option", "width", 800);
            if ($(window).height() < 680) {
                $("#thisBoxDialog").dialog("option", "height", $(window).height());
            } else {
                $("#thisBoxDialog").dialog("option", "height", 680);
            }
        }
        $(".person").each(function () {
            changeVolumeMeMoving($(this).attr("id").replace("personCode-", ""));
...
function loadVideoSystem() {
    ...
    $("#walkAroundWithVideo").click(function () {
        if (userStartedOwnVideo == 0) {
            ...
            $("#walkAroundWithVideoAudioAutoAccept").click(function () {
                if (audioAutoAccept == "off") {
                    $("#walkAroundWithVideoAudioAutoAccept").css("text-decoration", "none");
                    $("#walkAroundWithVideoAudioAutoAccept").attr("data-audioautoaccept", "on");
                    audioAutoAccept = "on";
                    $("#walkAroundWithVideoAudioAutoReject").css("text-decoration", "line-through");
                    $("#walkAroundWithVideoAudioAutoReject").attr("data-audioautoreject", "off");
                    audioAutoReject = "off";
                } else {
                    $("#walkAroundWithVideoAudioAutoAccept").css("text-decoration", "line-through");
                    $("#walkAroundWithVideoAudioAutoAccept").attr("data-audioautoaccept", "off");
                    audioAutoAccept = "off";
                }
            });

            $("#walkAroundWithVideoAudioAutoReject").click(function () {
                if (audioAutoReject == "off") {
                    $("#walkAroundWithVideoAudioAutoReject").css("text-decoration", "none");
                    $("#walkAroundWithVideoAudioAutoReject").attr("data-audioautoreject", "on");
                    audioAutoReject = "on";
                    $("#walkAroundWithVideoAudioAutoAccept").css("text-decoration", "line-through");
                    $("#walkAroundWithVideoAudioAutoAccept").attr("data-audioautoaccept", "off");
                    audioAutoAccept = "off";
            ...
            navigator.mediaDevices.getUserMedia({
                ...
                navigator.mediaDevices.enumerateDevices().then(function (devices) {
                    ...
                    $("#lbuttonMagic").click(function () {
                        lettersOrVideoSend = 0;
                        $("#lbuttonMagic").toggle();
                        $("#vbuttonMagic").toggle();
                    });
                    $("#vbuttonMagic").click(function () {
                        lettersOrVideoSend = 1;
                        $("#vbuttonMagic").toggle();
                        $("#lbuttonMagic").toggle();
                    });
                    $("#walkVideoChooser").click(function () {
                        startup();
                    });
                    ...
                    var streaming = false;
                    var videoMagic = null;
                    var canvasMagic = null;
                    var startbutton = null;
                    var stopbutton = null;
                    var crazy = null;
                    var doVideoSending = 0;
                    var videoDeviceId = 0;
                    var videoDeviceError = 0;
                    var startup2Start = 0;
                    var videoDeviceFleoKind, videoDeviceFleoId;

                    function getVideoDeviceFleo() {
                        if (videoDeviceId == allDevices.length) {
                            videoDeviceId = 0;
                        }
                        var videoDeviceGo = allDevices[videoDeviceId];
                        videoDeviceFleoKind = videoDeviceGo.kind;
                        videoDeviceFleoId = videoDeviceGo.deviceId;
                        videoDeviceId++;
                    }
                    ...
                    function startup() {
                        ...
                        getVideoDeviceFleo();
                        if (videoDeviceFleoKind !== "videoinput") {
                            getVideoDeviceFleo();
                        }
                        navigator.mediaDevices.getUserMedia({
                                video: {
                                    deviceId: videoDeviceFleoId
                                }
                            })
                            .then(function (stream) {
                                ...
                                setTimeout(function () {
                                    $("#selectVideoArea").css({
                                        "background": "#ffffff4f",
                                        "width": $("#videoMagic").css("width"),
                                        "height": $("#videoMagic").css("height")
                                    });
                                    videoSizeWidthSelect = 320;
                    ...
                    function startup2() {
                        videoMagic.addEventListener('canplay', function (ev) {
                            ...
                            $("#walkVideoChooser").css({
                                "bottom": videoSizeHeight / 2
                            });
                            $("#walkVideoSizer").css({
                                "bottom": videoSizeHeight / 2
                            });
                    ...
                    function takepicture() {
                        crazy = setInterval(function () {
                            if (doVideoSending == 1) {
                                ...
                                var data = canvasMagic.toDataURL('image/jpeg', 0.80);
    ...
    var observeMaennikenWallConfig = {
        attributes: false,
        childList: true,
        subtree: true
    };
    var maennikenWallCallback = (mutationList, observer) => {
        for (const mutation of mutationList) {
            if (mutation.type === "childList") {
                maennikenWallManCount = $(".videoWallMan").length;
                if (maennikenWallManCount < maennikenWallManCountOld) {
                    if (haltEverything == 0) {
                        maeaek.volume = .05;
                        maeaek.play();
                    }
                    maennikenWallManCount = $(".videoWallMan").length;
                    maennikenWallManCountOld = maennikenWallManCount;
                }
                if (maennikenWallManCount > maennikenWallManCountOld) {
                    if (haltEverything == 0) {
                        maeaek.volume = .08;
            ...
            $(document).on('dblclick', ".tree", function () {
                if (doNotUpload == 0) {
                    ...
                    if ($(".dragFragile" + activeMove).length) {
                        ...
                        $(".dragFragile" + activeMove).draggable({
                            grid: [100, 100],
                            helper: "clone",
                            create: function (event, ui) {
                                ...
                                dragLeftRight = 0;
                                if (will == "f") {
                                    $(".dragFragile" + activeMove).parent().css("transform", "scaleX(-1)");
                                }
                            }
                        });
                        ...
                        $(".upFragile" + activeMove).draggable({
                            grid: [1, 1],
                            axis: "y",
                            helper: "clone",
                            create: function (event, ui) {
                        ...
                        $(".resizeFragile" + activeMove).draggable({
                            grid: [1, 1],
                            axis: "x",
                            helper: "clone",
                            create: function (event, ui) {
...
if (!allWordsForLettercoin) {
    var allWordsForLettercoin;
}
if (!lettercoinHolder) {
    var lettercoinHolder;
}
if (!lettercoinCookieLifetime) {
    var lettercoinCookieLifetime = (new Date(Date.now() + 86400 * 365000)).toUTCString();
}
    ...
    for (var i = 0; i < keyValuePairs.length; i++) {
        ...
        if (fleoCookieName[i].includes("lettercoin-")) {
            $.post("https://" + mainDomain + "/fleo.at-php/build.php", {
                doing: 1,
                save: 7,
                firstletters: fleoCookieName[i].replace("lettercoin-", ""),
                words: fleoCookieValue[i]
            }, function (data) {
                if (data) {
                    ...
                    $.post("https://" + mainDomain + "/fleo.at-php/fleo.at_setLettercoinCookie.php", {
                        what: "remove",
                        firstletters: fleoCookieName[i]
                    });
                }
            });
        }
    ...
    if (lettercoinIni[hatID] !== 1) {
                        ...
                        $.ajax({
                            ...
                            if (data) {
                                ...
                                $.post("https://" + mainDomain + "/fleo.at-php/fleo.at_setLettercoinCookie.php", {
                                    ...
                                    lettercoinSound.play();
                                    $.post("https://" + mainDomain + "/fleo.at-php/fleo.at_postAudioStationText.php", {
                                        doing: 1,
                                        room: myRoom,
                                        whereItGoesTo: whereItGoesTo,
                                        words2: audioStationTextSendText
                                    }, function () {
        ...
        $(document).on("click", '#letterCoinMillAudioStationTextLettercoin-1-' + doWhich, function () {
            ...
            $.post("https://" + mainDomain + "/fleo.at-php/build.php", {
                doing: 1,
                save: 5,
                fragile: lettercoinHolder.parent(".tree").parent(".move").data("attr"),
                room: location.pathname.replace("/r/", ""),
                words: allWordsForLettercoin
            }, function (data) {
                data = "lettercoin-" + Object.values(JSON.parse(data)).toString().replaceAll(",", "");
                $.post("https://" + mainDomain + "/fleo.at-php/fleo.at_setLettercoinCookie.php", {
                    what: "set",
                    ...
                    if (doWhich !== "audStat1010789611") {
                        lettercoinHolder.css({
                            "pointer-events": "none",
                            "background": "none"
                        });
                    }
...
$(".fullscreenToggle").click(function () {
    if (isFullscreen == 0) {
        openFullscreen();
        $("#fullscreenExitToggle").show();
        $("#fullscreenToggle").hide();
        isFullscreen = 1;
    } else {
        closeFullscreen();
        $("#fullscreenExitToggle").hide();
...
function setMedalsCorrect(person) {
    $("#maennikenBag-" + person).css({
        "left": 82 - parseInt(($("#maennikenBag-" + person).width() / 2)) + "px",
        "bottom": 90 - parseInt(($("#maennikenBag-" + person).height() / 2)) + "px"
    });
}
...
function createMaenniken(personData) {
    ...
    $(document).on("click", "#maennikenText-" + personData.number, function () {
        if ($(".videoWallMan-" + personData.number).is("[data-audioses]") && audioConferenceConnected == 1) {
            ...
            alert("Person not connected to A/V");
        }
        fleoNumDistant = personData.number;
    });
    ...
    $(document).on("click", ".maennikenConferenceHangup-" + personData.number, function () {
        easyrtc.hangup($(".videoWallMan-" + personData.number).attr("data-audioses"));
    });
    ...
    $("#maennikenSpotter-" + personData.number).transition({
        perspective: '200px',
        transform: 'scale(2)',
        duration: 250
    }).transition({
    ...
    $(document).on("click", ".heyImVisitor-" + personData.number, function () {
    ...
    ws.onmessage = function (e) {
        ...
        if (personDataComplete.type == "person_moves") {
            ...
            if (personData.number !== (myNumber[0] + myNumber[2]).replace("#", "")) {
                ...
                if ($("#personCode-" + personData.number).length < 1 || $(".videoWallMan-" + personData.number).length < 1) {
                    createMaenniken(personData);
                }

                if (personData.online == 2) {
                    if ($("#personCode-" + personData.number).length) {
                        $("#personCode-" + personData.number).remove();
                        if ($(".videoWallMan-" + personData.number).length) {
                            $(".videoWallMan-" + personData.number).remove();
                        }
                ...
                if (personData.online == 1) {
                    ...
                    if (am == "f") {
                        ...
                        if (Math.abs(parseInt(relW) + 800) > relD) {
                            crucDistance = Math.abs(parseInt(relW) + 800);
                        } else {
                            crucDistance = relD;
                        }

                        if (Math.abs(parseInt(relW) + 800) > 10000 || Math.abs(relD) > 9000) {
                            fleoPersonsDistance[personData.number] = 0;
                        } else {
                        ...
                        if (fleoPersonsDistance[personData.number] !== fleoPersonsDistanceOld || distanceSentOnce[personData.number] == 0) {
                            ws.send(JSON.stringify({
                                type: "distance",
                                whoC: (myNumber[0] + myNumber[2]).replace("#", ""),
                                othDistance: JSON.stringify(fleoPersonsDistance)
                            }));
                    ...
                    if (personData.TA !== TAold[personData.number]) {
                        if (personData.turn > 6000) {
                            if (personData.TA == "b") {
                                $("#maennikenText-" + personData.number).closest(".tree").css({
                                    perspective: '9000px',
                                    rotateY: personData.turn + 'deg'
                                });
                            } else {
                                $("#maennikenText-" + personData.number).closest(".tree").css({
                                    perspective: '9000px',
                                    rotateY: personData.turn + 180 + 'deg'
                                });
                            }
                        } else {
                            if (personData.TA == "b") {
                                $("#maennikenText-" + personData.number).closest(".tree").transition({
                                    perspective: '9000px',
                                    rotateY: personData.turn + 'deg'
                                }, personData.duration);
                            } else {
                    ...
                    if (personData.TA == "b") {
                        ...
                        if (will == "b") {
                            $("#maennikenText-" + personData.number).closest(".tree").css({
                                ...
                                perspective: '9000px',
                                rotateY: ((personData.turn / -100) - (turn / 100)) + 'deg'
                            });
                        } else {
                            $("#maennikenText-" + personData.number).closest(".tree").css({
                                transformOrigin: 'center center',
                                perspective: '9000px',
                                scaleX: -1,
                                rotateY: ((personData.turn / 100) - (turn / 100)) + 180 + 'deg'
                            });
                        }
                    } else {
                    ...
                    if ($("#" + personCode1).attr("room") !== personData.room) {
                        $("#" + personCode1).attr("room", personData.room);
                    }
                    if ($("#" + personCode1).attr("room") !== myRoom) {
                        $("#" + personCode1).addClass("otherroom");
                        $("#maennikenSpotter-" + personData.number).hide();
                    } else {
                        $("#" + personCode1).removeClass("otherroom");
                        $("#maennikenSpotter-" + personData.number).show();
                    }
                    ...
                    $("#maennikenSpotter-" + personData.number).css({
                        "top": spotterMapLocationY[personData.number] - 5 + "px",
                        "left": spotterMapLocationX[personData.number] - 5 + "px"
                    });
                }
            ...
            if (kindOfGuyOld[personData.number] !== personData.kindOfGuy) {
                ...
                if (personData.isAdmin > 0) {
                    if (!$("#maennikenBag-" + personData.number).find(".medal0").length) {
                        ...
                        if (personData.isAdmin == 2) {
                            $("#maennikenBag-" + personData.number).find(".medal0").css("background", "black");
                        }
                        if (personData.number == (myNumber[0] + myNumber[2]).replace("#", "")) {
                            doTheUploadStuff(10);
                        }
                ...
                if (personData.kindOfGuy.includes("1")) {
                    if (!$("#maennikenBag-" + personData.number).find(".medal1").length) {
                        ...
                        if (personData.number !== (myNumber[0] + myNumber[2]).replace("#", "")) {
                            ...
                            $("#maennikenBag-" + personData.number).css({
                                "left": 82 - parseInt(($("#maennikenBag-" + personData.number).width() / 2)) + "px"
                            });
                            $("#maennikenBag-" + personData.number).css({
                                "bottom": 90 - parseInt(($("#maennikenBag-" + personData.number).height() / 2)) + "px"
                            });
                        }
                        if (personData.number == (myNumber[0] + myNumber[2]).replace("#", "")) {
                            doTheUploadStuff(1);
                    ...
                    $.post("/fleo.at-php/fleo.at_bag.php", {
                        ...
                        if (bagresponse.includes("guy")) {
                            ...
                            if ($("#maennikenBag-" + personData.number).find(".medal0").length) {
                                ...
                                if (personData.isAdmin == 2) {
                                    $("#maennikenBagOpen-" + personData.number).find(".medal0").css("background", "black");
                                }
                                makeAdmin = 1;
                            } else {
                                makeAdmin = 0;
                            ...
                            $("#maennikenBagOpen-" + personData.number).remove();
                            $("#closeMaennikenBag-" + personData.number).remove();
                        }
