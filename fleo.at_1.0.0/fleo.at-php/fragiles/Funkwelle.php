...
if ($mpMoveW == 1) {
    ...
    if ($FragileW == -25500 && $FragileD == 1000) { $FragileW = -25500; $FragileD = 1000; $mpMoveW = 0; $mpChange = 1; } else { $mpChange = 0; }

} else {

    if ($FragileW < -101400) { $FragileW += 300; if ($FragileW > -101400) {  $FragileW = -101400; }}
    if ($FragileW > -101400) { $FragileW -= 300; if ($FragileW < -101400) {  $FragileW = -101400; }}
    if ($FragileD < -4000) { $FragileD += 300; if ($FragileD > -4000) {  $FragileD = -4000; }}
    if ($FragileD > -4000) { $FragileD -= 300; if ($FragileD < -4000) {  $FragileD = -4000; }}

    if ($FragileW == -101400 && $FragileD == -4000) { $FragileW = -101400; $FragileD = -4000; $mpMoveW = 1; $mpChange = 1; } else { $mpChange = 0; }
}
...
if ($row["program"] == $meIam) {
    ...
    */
    ...
    if ($i > 79) { 
        exec('php ' . $fleoPathAbs . '/fleo.at_1.0.0/fleo.at-php/fragiles/Funkwelle.php 1 '.$id.' '.$room.' > /dev/null &');
        $goBaby = 0;
    }
    if ($row["program"] !== $meIam) {
        $goBaby = 0;
