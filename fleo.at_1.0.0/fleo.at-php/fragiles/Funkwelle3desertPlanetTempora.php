...
if ($mpMoveW == 1) {
    ...
    if ($FragileW < 4500) { $FragileW += 300; if ($FragileW > 4500) {  $FragileW = 4500; }}
    if ($FragileW > 4500) { $FragileW -= 300; if ($FragileW < 4500) {  $FragileW = 4500; }}
    if ($FragileD < -300) { $FragileD += 300; if ($FragileD > -300) {  $FragileD = -300; }}
    if ($FragileD > -300) { $FragileD -= 300; if ($FragileD < -300) {  $FragileD = -300; }}

    if ($FragileW == 4500 && $FragileD == -300) { $FragileW = 4500; $FragileD = -300; $mpMoveW = 0; $mpChange = 1; } else { $mpChange = 0; }

} else {

    if ($FragileW < -40000) { $FragileW += 300; if ($FragileW > -40000) {  $FragileW = -40000; }}
    if ($FragileW > -40000) { $FragileW -= 300; if ($FragileW < -40000) {  $FragileW = -40000; }}
    if ($FragileD < 1000) { $FragileD += 300; if ($FragileD > 1000) {  $FragileD = 1000; }}
    if ($FragileD > 1000) { $FragileD -= 300; if ($FragileD < 1000) {  $FragileD = 1000; }}

    if ($FragileW == -40000 && $FragileD == 1000) { $FragileW = -40000; $FragileD = 1000; $mpMoveW = 1; $mpChange = 1; } else { $mpChange = 0; }
}
...
if ($row["program"] == $meIam) {
    ...
    */
    ...
    if ($i > 79) { 
        exec('php ' . $fleoPathAbs . '/fleo.at_1.0.0/fleo.at-php/fragiles/Funkwelle3desertPlanetTempora.php 1 '.$id.' '.$room.' > /dev/null &');
        $goBaby = 0;
    }
    if ($row["program"] !== $meIam) {
        $goBaby = 0;
